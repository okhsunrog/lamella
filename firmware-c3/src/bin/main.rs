...
/// Worker task for outgoing data. A short final USB packet terminates a
/// transfer by itself; only flush exact-64-byte segments to generate the ZLP
/// they otherwise lack. Avoiding redundant flushes also avoids committing an
/// already-empty hardware FIFO a second time.
#[task]
